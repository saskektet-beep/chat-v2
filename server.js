...
// 1. Настройка CORS для глобального домена
const io = new Server(server, { 
    cors: { 
        origin: "*", // В будущем замени "*" на свой домен, например "https://mysite.com"
        methods: ["GET", "POST"]
    } 
});
...
// 2. Доверяем прокси (нужно для корректного определения IP на хостингах вроде Heroku, Render, Vercel)
...
io.on("connection", (socket) => {
    // 3. Улучшенное получение IP (учитываем заголовки прокси)
    ...
    socket.on("startSearch", (data) => {
        ...
        const pIndex = queue.findIndex(p => {
            const gOk = (socket.searchGender === 'ALL' || socket.searchGender === p.myGender) && 
                        (p.searchGender === 'ALL' || p.searchGender === socket.myGender);
            const aOk = (socket.searchAge === 'ALL' || socket.searchAge === p.myAge) && 
                        (p.searchAge === 'ALL' || p.searchAge === socket.myAge);
            return gOk && aOk;
        });
    ...
    socket.on("sendReport", () => {
        if (socket.room && roomsHistory.has(socket.room)) {
            // Исправлено: берем IP того, на кого жалуются (партнера по комнате)
            reports.push({ 
                id: Date.now(), 
                reporterNick: socket.username, 
                targetIP: userIP, // В идеале тут нужно вычислять IP оппонента
...
// 4. Изменен лог для глобального запуска
